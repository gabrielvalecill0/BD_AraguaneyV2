...
public class EnfermosPdf {
    
     List<Enfermos> lista;

    public EnfermosPdf(List<Enfermos> lista) {
        super();
        this.lista = lista;
    }
    ...
    private void escribirCabeceraDeLaTabla(PdfPTable tabla) {
        ...
        celda.setPhrase(new Phrase("ENFERMEDAD", fuente));
        tabla.addCell(celda);
    ...
    private void escribirDatos(PdfPTable tabla) {
        ...
        for (Enfermos dato : lista) {

            tabla.addCell(String.valueOf(dato.getIdEnfermos()));
            tabla.addCell(String.valueOf(dato.getDatosPersonales().getCedula()));
            tabla.addCell(dato.getEnfermedad());
    ...
    public void exportar(HttpServletResponse response) throws IOException, DocumentException {
        ...
        Paragraph titulo = new Paragraph("Reporte Enfermos", fuente);
